...
namespace OCS_Test_Assignment.Controllers
{
    ...
    public class OrdersController : ControllerBase, IController<OrderDTO>
    {
            ...
            else
            {
                ...
                */
                if (await _dbOperations.CreateAsync(newObj) == true)
                {
                    ...
                    var fullUrl = baseUrl + $"/api/orders/{obj.id.ToString()}";
        ...
        public async Task<IActionResult> Update([FromRoute] Guid orderId, [FromBody] OrderDTO obj)
        {
            var oldOrder = await _dbOperations.GetByIDAsync(orderId);
